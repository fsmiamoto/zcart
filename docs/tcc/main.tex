...
\begin{figure}[H]
    ...
    \fonte{Michigan Scientific Corporation (2022)}
	\label{fig:gauge1}
\end{figure}
...
In practice, measuring the resistance variations are minute and can be
difficult to mesure. Additionally, it is usually desirable to have as the
...
\begin{quote}
Now that the process of strain measurements has been established, the next step when using strain gauges is getting useful data.The strain gauge must be connected to an electrical circuit that is capable of accurately responding to the minute changes in resistance associated with strain. Multiple strain gauges can be used in a divided bridge circuit to measure small changes in electrical resistance. This is called a Wheatstone bridge. In a Wheatstone bridge configuration, an excitation voltage is applied across the circuit, and the output voltage is measured across two points in the middle of the bridge. When there is no load acting on the load cell, the Wheatstone bridge is balanced and there is zero output voltage. Any small change in the material under the strain gauge results in a change in the resistance of the strain gauge as it deforms with the material. This causes the bridge to be thrown out of balance, resulting in a change in the output voltage. As stated earlier, the resistance change is minute, which means that signal amplification is often needed to properly determine changes. The amplification process strengthens the strain signal changes; however, it also leads to more unwanted noise also being detected in the signal. Signal conditioning filters out the excess noise, ensuring accurate and understandable data. 
\end{quote}

It can be shown then that the relation between $V_{out}$ and $V_{in}$, shown on Figure \ref{fig:gauge2}, can be
calculated as \cite{Stefanescu}:

\begin{equation}
    S = \frac{V_{out}}{V_{in}} = k\epsilon
\end{equation}

The output voltage $V_{out}$ will be then linearly proportional to the amount of strain applied.

\section{Load Cell}

Load cells are designed to focus stress through beam elements where strain gauges are located.
...
The first important step, is the \textit{frame diff}. This step calculates the
a dictionary of the object difference, called \textit{diff}, by comparing the
stored product dictionary and the received list of frame objects from the Main
Thread.
...
For example, if the can illustraded in Figure \ref{fig:diff} weights an expected 400 grams, the weight
...
One of the import aspects when developing a marketable product is its \textbf{cost}.

A more detailed overview of the items and their cost is shown on Table \ref{tbl:cost}.
...
\begin{table}[H]
	...
	\begin{tabular}{c c c}
        ...
        \hline
	\end{tabular}
    \caption[Items used on the prototype and their approximate retail cost in October 2022]{Items used on the prototype and their retail cost in October 2022}
	\fonte{Own work}
    \label{tbl:cost}
\end{table}
...
product to provide the expected battery life but that can come with a trade off
of additional weight and cost, which might be considered undesired
characteristics from the end user and grosser perspective, respectively.

Additionally, it would be necessary to developed a practical mechanism to
allow the carts to be charged such as a docking mechanism or even by wireless charging
\cite{Treffers2015}.
