...
consolidated pre-trained Network- usually being the hidden layers, which are the feature extractors- 
when training custom models, this way only some layers of the Network- usually the last layers, which 
are the decisors- effectively have to be tuned.

The biggest advantages of Transfer Learning are that, because it allows for training only specific 
layers of an architecture to get a custom model for inference, the training process is much faster
it would be for a whole network, because you do not need to compute all the weights and biases 
of your custom model; and also that you typically do not need to have a lot of pictures for training
good performing models, as you would be reusing pre-trained weights from a model that was already
trained on larger datasets\cite{CS231N}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{./images/image_preprocessing.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{./images/efficientdet-d0-resized-whole-loss.png}
	\caption[Train and Validation Classification Loss for the EfficientDet-D0 Model that 
		was whole-trained and took resized images as an input]{Train and Validation Classification Loss 
		for the EfficientDet-D0 Model that was whole-trained and took resized images as an input}
	\fonte{Own work}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{./images/efficientdet-d1-resized-whole-loss.png}
	\caption[Train and Validation Classification Loss for the EfficientDet-D1 Model that 
		was whole-trained and took resized images as an input]{Train and Validation Classification Loss 
		for the EfficientDet-D1 Model that was whole-trained and took resized images as an input}
	\fonte{Own work}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{./images/efficientdet-d2-resized-whole-loss.png}
	\caption[Train and Validation Classification Loss for the EfficientDet-D2 Model that 
		was whole-trained and took resized images as an input]{Train and Validation Classification Loss 
		for the EfficientDet-D2 Model that was whole-trained and took resized images as an input}
	\fonte{Own work}
\end{figure}
