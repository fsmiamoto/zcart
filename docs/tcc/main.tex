...
%abstract
\begin{abstract}
The recent advancements in artificial intelligence have already impact many
...
markets such as Brazil, due to historical economical barriers. In that
regard, this work analyses the current market and develops a prototype that
...
In the last decades, inferences using a Deep Neural Network (\sigla{DNN}{Deep
Neural Network}) executed on general purpose hardware such as \sigla{CPU}{Central
Processing Unit}s or specialized hardware but target for DNNs such as
...
contemporary hardware of paired with reduced power consumption.
...
in resistance that when strain is applied to the sensor \cite{Michigan2020}.
...
use mechanical packages that embedded the strain gauges for weight sensing
